...
These files contains the data and analysis scripts for "Automatic Responses to Acoustically Rough Intervals" concerning the reaction time data. All data files are available at OSF repository: [https://osf.io/zmjpd/](https://osf.io/zmjpd/).
...
Separate files produce the predictions by computational models of roughness and harmonicity. The model predictions are also compared to each other and empirical data (the results rating studies) and the way the predictions are created (separation into High and Low) are also created by the scripts. Also, the stimulus visualisations are included in the scripts (spectrum plots).
